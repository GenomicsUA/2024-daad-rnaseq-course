...
res_de %>%
  ...
  dplyr::slice(1:50, (n() - 49):n())
```


```{r}
top_100_ashr <- rownames(res_de %>%
  arrange(log2FoldChange) %>%
  dplyr::slice(c(1:50, (n() - 49):n())))
top_100_deseq2 <- rownames(res_deseq2 %>%
  arrange(log2FoldChange) %>%
  dplyr::slice(c(1:50, (n() - 49):n())))

top_1000_ashr <- rownames(res_de %>%
  arrange(log2FoldChange) %>%
  dplyr::slice(c(1:500, (n() - 499):n())))
top_1000_deseq2 <- rownames(res_deseq2 %>%
  arrange(log2FoldChange) %>%
  dplyr::slice(c(1:500, (n() - 499):n())))

top_2000_ashr <- rownames(res_de %>%
  arrange(log2FoldChange) %>%
  dplyr::slice(c(1:1000, (n() - 999):n())))
top_2000_deseq2 <- rownames(res_deseq2 %>%
  arrange(log2FoldChange) %>%
  dplyr::slice(c(1:1000, (n() - 999):n())))
```

```{r}
setdiff(top_100_ashr, top_100_deseq2)

length(setdiff(top_100_ashr, top_100_deseq2))
length(setdiff(top_1000_ashr, top_1000_deseq2))
length(setdiff(top_2000_ashr, top_2000_deseq2))
```
```{r}
res_de <- res_de %>%
