...
```{r}
logFC <- de_results$log2FoldChange
names(logFC) <- de_results$entrezgene_id

logFC_2 <- de_results_2$log2FoldChange
names(logFC_2) <- de_results_2$entrezgene_id
```

```{r}
pathview(gene.data = logFC_2, 
         pathway.id = "mmu04657", 
         species = "mmu", 
         kegg.native = T)
```
...
top_degree_genes <- centrality_scores %>%
  arrange(desc(degree)) %>%
  head(10)

top_closeness_genes <- centrality_scores %>%
  arrange(desc(closeness)) %>%
  head(10)

top_betweenness_genes <- centrality_scores %>%
