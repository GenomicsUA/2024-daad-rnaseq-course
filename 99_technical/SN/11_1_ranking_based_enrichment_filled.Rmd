...
de_results_2 <- read.csv(here("03_outputs", "07_deseq_ashr_DE_TAACIRvsTAAHC_all.csv"), row.names = 1)
...
```{r}
gseWP_res_025 <- gseWP(genes_list, 
...
```{r}
kegg_result <- gseKEGG(genes_list, organism = "mmu", by = "DOSE", nPerm = 1000)
```
...
```{r}
gseWP_res_025_genename <- setReadable(gseWP_res_025, 
                                      'org.Mm.eg.db', 
                                      'ENTREZID')
...
```{r}
ck <- compareCluster(geneCluster = de_genes_all_experiments, 
                     fun = "enrichGO", Org.Db = "org.Mm.eg.db")
